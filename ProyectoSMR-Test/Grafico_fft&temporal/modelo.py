...
class Measure():
    ...
    def __init__(self, widgets):
        ...
        self.num_temporal = 1
        self.cont_ensayos = 1
        self.samples = np.arange(0, SAMPLES_ADC, 1)
    ...
    def init_ensayo(self):
        ...
        ### OBTENER PROCENTAJE DE PANTALLAS FFT Y TEMPORAL
    ...
    def save_image(self):
        ...
        name_fft_ant = os.path.join("capture", f"FFT_ant_{self.num_temporal}.tiff")

        self.widgets.grafica.fig.savefig(name_temporal_ant)
        self.widgets.grafica2.fig.savefig(name_fft_ant)
    ...
    def widgets_config(self):
        ...
        """
        self.widgets.ui.dial_sample_temp.setEnabled(False)
        self.widgets.ui.dial_volt_init_temp.setEnabled(False)
        self.widgets.ui.dial_volt_fin_temp.setEnabled(False)
        self.widgets.ui.dial_freq_init_fft.setEnabled(False)
        self.widgets.ui.dial_freq_fin_fft.setEnabled(False)  
        self.widgets.ui.dial_amp_fin_fft.setEnabled(False)   
        """
    ...
    def widgets_ensayo(self):
        ...
        """
        self.widgets.ui.dial_sample_temp.setEnabled(True)
        self.widgets.ui.dial_volt_init_temp.setEnabled(True)
        self.widgets.ui.dial_volt_fin_temp.setEnabled(True)
        self.widgets.ui.dial_freq_init_fft.setEnabled(True)
        self.widgets.ui.dial_freq_fin_fft.setEnabled(True)  
        self.widgets.ui.dial_amp_fin_fft.setEnabled(True)
        """
    ...
    def reset_widgets(self):
        ...
        self.widgets.grafica.ax.set_title("Grafico Temporal")
        self.widgets.grafica.upgrade_graph_temp(self.samples, np.zeros(SAMPLES_ADC))

        self.widgets.grafica2.ax.clear()
        self.widgets.grafica2.ax.set_title("Grafico FFT")
        self.widgets.grafica2.upgrade_graph_fft(self.freq, np.zeros(SAMPLES_FFT))
    ...
    def data_recive(self):
        ...
        if self.mqtt_obj.mcp_ant:
            ...
            self.widgets.grafica.ax.set_title("Grafico temporal")
            #   MODIFICAR PARAMETROS  A PASAR PARA GRAFICO TEMPORAL
            self.widgets.grafica.upgrade_graph_temp(self.samples, self.mcp_ant)
        ...
        if self.mqtt_obj.fft_ant:
            ...
            self.widgets.grafica2.ax.clear()
            self.widgets.grafica2.ax.set_title("Grafico FFT")
            self.widgets.grafica2.upgrade_graph_fft(self.freq, self.fft_ant)
